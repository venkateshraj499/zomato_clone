...
function QuickSearchItem(props) {
  ...
  var handleNavigate = (mealTypeId) => {
    ...
    history.push(
      locId
        ? `/filter?mealtype=${mealTypeId}&location=${locId}`
        : `/filter?mealtype=${mealTypeId}`
    );
  };

  return (
    <div
      className=" col-xl-3 col-lg-5 col-md-5 col-sm-10 qs-options"
      onClick={() => handleNavigate(qsData.meal_type)}
    >
      <link
